...
/** When true (e.g. Vercel static), speaking form uses mailto instead of API. */
const MARKETING_MODE = import.meta.env.VITE_MARKETING_MODE === "true";

function buildMailtoHref(data: SpeakingInviteValues): string {
  const subject = encodeURIComponent(`Speaking invitation – ${data.organisation}`);
  const lines = [
    ...
    data.audienceType ? `Audience: ${data.audienceType} (${data.audienceSize || "—"})` : null,
    data.topicInterests?.length ? `Topics: ${data.topicInterests.join(", ")}` : null,
    data.topicNotes ? `Topic notes: ${data.topicNotes}` : null,
...
export function SpeakingFormContent({ onSuccess }: { onSuccess: () => void }) {
  ...
  function handleSpeakingSubmit(data: SpeakingInviteValues) {
    if (MARKETING_MODE) {
      trackEvent("form_submit", { form_name: "speaking_invitation", mode: "mailto" });
      window.location.href = buildMailtoHref(data);
      toast({
        title: "Opening your email",
        description: "Send the message to submit your invitation. I'll be in touch within 1–2 business days.",
      });
      form.reset();
      onSuccess();
    } else {
  ...
  return (
    <>
      ...
      />
        ...
        <Form {...form}>
          <form
            onSubmit={form.handleSubmit(handleSpeakingSubmit)}
            className="space-y-8"
          >
